...
RETURN DISCIPLINE_TABLE PIPELINED;

FUNCTION GET_DISCIPLINE_TEACHER(
    DSCPLN_NAME IN DISCIPLINE.DISCIPLINE_NAME%TYPE
...
CREATE OR REPLACE PACKAGE BODY DISCIPLINE_PACKAGE IS
    ...
    BEGIN
    FOR PIECE IN (
        SELECT DISTINCT TEACHER.TEACHER_NAME
        FROM TEACHER JOIN HOMEWORK ON
        TEACHER.CONTRACT_ID = HOMEWORK.CONTRACT_ID
        WHERE HOMEWORK.DISCIPLINE_NAME = DSCPLN_NAME
    ) LOOP
        PIPE ROW (PIECE);
    END LOOP;
    END GET_DISCIPLINE_TEACHER;
